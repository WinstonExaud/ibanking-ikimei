...
import {
  ...
  Building2, TrendingUp, Home, Menu, X, ChevronRight, Bell
} from 'lucide-react';
  ...
  return (
    <>
      ...
      <aside className={`
        ...
        w-64 bg-white border-r border-surface-border
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-all font-inter text-sm font-medium
                  ${isActive
                    ...
                    : 'text-gray-500 hover:bg-surface-bg hover:text-gray-800'
                  }`}
              >
                <Icon size={17} />
                <span>{label}</span>
...
// ─── Topbar ───────────────────────────────────────────────────────────────────
function Topbar({ active, onMenuClick, onRefresh }) {
  ...
  return (
    <header className="bg-white border-b border-surface-border px-5 py-3.5 flex items-center justify-between sticky top-0 z-20">
      <div className="flex items-center gap-3">
        <button
          onClick={onMenuClick}
          className="lg:hidden w-9 h-9 flex items-center justify-center rounded-xl hover:bg-surface-bg text-gray-500"
        >
          <Menu size={20} />
        </button>
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={onRefresh}
          className="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-surface-bg text-gray-400 hover:text-gray-700 transition-all"
        >
          <RefreshCw size={17} />
        </button>
...
export default function ClientDashboard() {
  ...
  useEffect(() => {
    ...
    if (isAccount) {
      const u1 = onAccountsSnapshot(setAccounts);
      const u2 = onTransactionsSnapshot(txs => {
        const mine = txs.filter(t =>
          t.fromAccountId === userDoc.accountDocId ||
          t.toAccountId   === userDoc.accountDocId
        );
  ...
  const linkedAccount = isAccount
    ? accounts.find(a => a.id === userDoc?.accountDocId)
    : null;

  const balance      = isAccount ? safeBalance(linkedAccount?.balance) : safeBalance(userDoc?.balance);
  const limit        = isAccount ? safeBalance(linkedAccount?.limit)   : null;

  const totalSent = isAccount
    ? transactions.filter(t => t.type === 'transfer' && t.fromAccountId === userDoc.accountDocId)
  ...
  const totalDeposited = isAccount
    ? transactions.filter(t => t.type === 'deposit').reduce((s, t) => s + safeBalance(t.amount), 0)
    : null;

  const limitUsedPct = limit ? Math.min(100, (totalSent / limit) * 100) : 0;

  const cardAccount = isAccount
    ? { name: linkedAccount?.name || userDoc?.name, balance, accountId: userDoc?.accountId, color: linkedAccount?.color || '#2563EB' }
    : { name: userDoc?.name, balance, accountId: userDoc?.accountId, color: '#2563EB' };

  const chartData = buildMonthlyChart(transactions, isAccount, userDoc);
            ...
            <div className="animate-fade-in max-w-5xl mx-auto">
              ...
              <div className="mb-6">
                ...
                <p className="text-gray-400 font-inter text-sm mt-0.5">
                  {isAccount ? 'Your business account overview' : 'Your personal balance overview'}
                </p>
              </div>
                  ...
                  <div className="card">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <button
                        ...
                        className="text-xs text-primary-500 font-semibold font-inter hover:underline"
                      >
                        See all
                      </button>
                    </div>
                    {loading ? (
                      ...
                      <div className="divide-y divide-surface-border/60">
                        {recentTx.map(tx => (
                          <TxRow key={tx.id} tx={tx} isAccount={isAccount} myAccountDocId={userDoc?.accountDocId} />
                        ))}
                      </div>
                    )}
                  </div>
                  ...
                  {isAccount && limit > 0 && (
                    <div className="card">
                      ...
                      <div className="bg-gray-100 rounded-full h-2 mb-2">
                        <div
                          className={`h-2 rounded-full transition-all ${limitUsedPct > 80 ? 'bg-red-500' : 'bg-amber-400'}`}
